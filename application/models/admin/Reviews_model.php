...
class Reviews_model extends CI_Model
{
  ...
  private $_order = array('r.review_id' => 'asc');
  ...
  public function rules($context)
  {
    switch ($context) {
      case 'add':
        ...
        break;
      case 'edit':
        $params_check_available_user_ignore = serialize([$this->input->post('content_id'), $this->input->post('review_id')]);
        $params_check_available_content_ignore = serialize([$this->input->post('user_id'), $this->input->post('review_id')]);
  ...
  {
    return $this->db->get_where($this->_table, ["review_id" => $id])->row();
  }
  ...
  public function save()
  {
    ...
    $this->review_text = $post["review_text"];
  ...
  {
    $post = $this->input->post();
    $reviews = $this->getById($post["review_id"]);

    $this->review_id = $post["review_id"];
    $this->user_id = $post["user_id"] ?? $reviews->user_id;
    $this->content_id = $post["content_id"] ?? $reviews->content_id;
    $this->rating = $post["rating"] ?? $reviews->rating;
    $this->review_text = $post["review_text"] ?? $reviews->review_text;
    $this->created_at = $reviews->created_at;

    return $this->db->update($this->_table, $this, array('review_id' => $post['review_id']));
  }

  public function delete($id)
  {
    return $this->db->delete($this->_table, array("review_id" => $id));
  }
