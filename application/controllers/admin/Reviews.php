...
class Reviews extends CI_Controller
{
  ...
  public function add()
  {
    ...
    $reviews = $this->reviews_model;
    $validation = $this->form_validation;
    $validation->set_rules($reviews->rules('add'));
  ...
  public function edit($id = null)
  {
    ...
    if (!$this->reviews_model->getById($id)) return show_404();
    ...
    $reviews = $this->reviews_model;
    $validation = $this->form_validation;
    $validation->set_rules($reviews->rules('edit'));
  ...
  public function delete($id)
  {
    ...
    $reviews_data = $this->reviews_model->getById($id);

    if (!$reviews_data) return show_404();

    if ($this->input->post('nickname_confirm_delete') == $reviews_data->nickname) :
  ...
  public function ajax_list()
  {
    ...
    $list = $this->reviews_model->get_datatables();
    $result = array();
    ...
    foreach ($list as $reviews) {
      ...
      $endpoint = $this->_path_prefix . $this->_context . '/ajax_get_by_id/' . $reviews->review_id;

      $row[] = $reviews->review_id;
      $row[] = $reviews->username;
      $row[] = $reviews->title;
      $row[] = $reviews->rating;
      $row[] = $reviews->created_at;
      $row[] = '
        <a id="' . $this->_context . '-edit-' . $reviews->review_id . '" type="button" class="btn btn-success btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-edit`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `edit`, ' . $reviews->review_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-edit"><i class="fa fa-edit"></i> Edit</a>
        <a id="' . $this->_context . '-delete-' . $reviews->review_id . '" type="button" class="btn btn-danger btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-delete`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `delete`, ' . $reviews->review_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-delete"><i class="fa fa-trash"></i> Delete</a>
      ';
    ...
    $output = array(
      ...
      "recordsTotal" => $this->reviews_model->count_all(),
      "recordsFiltered" => $this->reviews_model->count_filtered(),
      "data" => $result,
    );
  ...
  public function ajax_get_by_id($id)
  {
    ...
    $row = $this->reviews_model->getById($id);
  ...
  private function page_meta_data()
  {
    ...
    $data["content"]["table"][$this->_context]["columns"] = $this->reviews_model->columns();

    $data["content"]["reviews"]["data"] = $this->reviews_model->getAll();
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form id="modal-' . $this->_context . '-add-form" action="' . base_url($this->_path_prefix . $this->_context . "/add") . '" method="post" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="card-body">
                      <input type="hidden" class="form-control" id="add_review_id" name="review_id">
                      ...
                      <div class="form-group">
                        <label for="rating">Rating <small>( 1 - 10 )</small> *</label>
                        <input type="number" class="form-control" id="add_rating" name="rating" value="' . set_value("rating") . '" placeholder="Enter rating" min="0" max="10" step=".01" autocomplete="off">
                        ' . form_error("rating", "<small id='add_rating-error' class='text-danger'>", "</small>") . '
                      </div>
                      <div class="form-group">
                        <label>Review Text *</label>
                        <textarea class="form-control" id="add_review_text" name="review_text" rows="3" placeholder="Enter review text">' . set_value("review_text") . '</textarea>
                        ' . form_error("review_text", "<small id='add_review_text-error' class='text-danger'>", "</small>") . '
                      </div>
                    </div>
                  </form>
                </div>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form id="modal-' . $this->_context . '-edit-form" action="' . base_url($this->_path_prefix . $this->_context . "/edit") . '" method="post" enctype="multipart/form-data" autocomplete="off">
                    <input type="hidden" name="' . $data["content"]["csrf"]["name"] . '" value="' . $data["content"]["csrf"]["hash"] . '">
                    <div class="card-body">
                      <input type="hidden" class="form-control" id="edit_review_id" name="review_id">
                      <div class="form-group">
                        <label>User *</label>
                      ...
                      <div class="form-group">
                        <label for="rating">Rating <small>( 1 - 10 )</small> *</label>
                        <input type="number" class="form-control" id="edit_rating" name="rating" value="' . set_value("rating") . '" placeholder="Enter rating" min="0" max="10" step=".01" autocomplete="off">
                        ' . form_error("rating", "<small id='edit_rating-error' class='text-danger'>", "</small>") . '
                      </div>
                      <div class="form-group">
                        <label>Review Text *</label>
                        <textarea class="form-control" id="add_review_text" name="review_text" rows="3" placeholder="Enter review text">' . set_value("review_text") . '</textarea>
