...
class Actor extends CI_Controller
{
  ...
  public function add()
  {
    ...
    $actor = $this->actor_model;
    $validation = $this->form_validation;
    $validation->set_rules($actor->rules('add'));
  ...
  public function edit($id = null)
  {
    ...
    if (!$this->actor_model->getById($id)) return show_404();
    ...
    $actor = $this->actor_model;
    $validation = $this->form_validation;
    $validation->set_rules($actor->rules('edit'));
  ...
  public function delete($id)
  {
    ...
    $actor_data = $this->actor_model->getById($id);

    if (!$actor_data) return show_404();

    if ($this->input->post('nickname_confirm_delete') == $actor_data->nickname) :

      if ($this->actor_model->delete($id)) :
  ...
  public function ajax_list()
  {
    ...
    $list = $this->actor_model->get_datatables();
    $result = array();
    ...
    foreach ($list as $actor) {
      ...
      $endpoint = $this->_path_prefix . $this->_context . '/ajax_get_by_id/' . $actor->actor_id;
      ...
      $row[] = $actor->last_name;
      $row[] = '
        <a id="' . $this->_context . '-edit-' . $actor->actor_id . '" type="button" class="btn btn-success btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-edit`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `edit`, ' . $actor->actor_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-edit"><i class="fa fa-edit"></i> Edit</a>
        <a id="' . $this->_context . '-delete-' . $actor->actor_id . '" type="button" class="btn btn-danger btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-delete`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `delete`, ' . $actor->actor_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-delete"><i class="fa fa-trash"></i> Delete</a>
      ';
    ...
    $output = array(
      ...
      "recordsTotal" => $this->actor_model->count_all(),
      "recordsFiltered" => $this->actor_model->count_filtered(),
      "data" => $result,
    );
  ...
  public function ajax_get_by_id($id)
  {
    ...
    $row = $this->actor_model->getById($id);
  ...
  public function unique_nickname_ignore($nickname, $excludeId)
  {
    ...
    $this->db->where('actor_id !=', $excludeId);
  ...
  private function page_meta_data()
  {
    ...
    $data["content"]["table"][$this->_context]["columns"] = $this->actor_model->columns();

    $data["content"]["actor"]["data"] = $this->actor_model->getAll();
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form id="modal-' . $this->_context . '-add-form" action="' . base_url($this->_path_prefix . $this->_context . "/add") . '" method="post" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="card-body">
                      <input type="hidden" class="form-control" id="add_actor_id" name="actor_id">
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form id="modal-' . $this->_context . '-edit-form" action="' . base_url($this->_path_prefix . $this->_context . "/edit") . '" method="post" enctype="multipart/form-data" autocomplete="off">
                    <input type="hidden" name="' . $data["content"]["csrf"]["name"] . '" value="' . $data["content"]["csrf"]["hash"] . '">
                    <div class="card-body">
                      <input type="hidden" class="form-control" id="edit_actor_id" name="actor_id">
                      <div class="form-group">
                        <label for="nickname">Nickname *</label>
