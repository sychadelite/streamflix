...
class Series extends CI_Controller
{
  ...
  public function add()
  {
    ...
    $series = $this->series_model;
    $validation = $this->form_validation;
    $validation->set_rules($series->rules('add'));
    ...
    if (!empty($_FILES[$this->_file[0]]['name'])) :
      ...
      if ($uploadFile['status']) {
        ...
        $series->cover_image = $uploadFile['data']['relative_path'];
      } else {
  ...
  public function edit($id = null)
  {
    ...
    if (!$this->series_model->getById($id)) return show_404();
    ...
    $series = $this->series_model;
    $validation = $this->form_validation;
    $validation->set_rules($series->rules('edit'));
    ...
    if (!empty($_FILES[$this->_file[0]]['name'])) :
      ...
      if ($uploadFile['status']) {
        $old_path = $series->getById($id)->cover_image;
  ...
  public function delete($id)
  {
    ...
    $series_data = $this->series_model->getById($id);

    if (!$series_data) return show_404();

    if ($this->input->post('name_confirm_delete') == $series_data->name) :

      if ($this->series_model->delete($id)) :
  ...
  public function ajax_list()
  {
    ...
    $list = $this->series_model->get_datatables();
    $result = array();
    ...
    foreach ($list as $series) {
      ...
      $endpoint = $this->_path_prefix . $this->_context . '/ajax_get_by_id/' . $series->content_id;

      $row[] = $series->content_id;
      $row[] = $series->title;
      $row[] = $series->release_year;
      $row[] = $series->duration;
      $row[] = $series->description;
      $row[] = $series->cast;
      $row[] = '
        <div style="width: 7rem; height: 7rem; border-radius: 10px; overflow: hidden;">
          <img src="' . ($series->cover_image ?? base_url('assets/vendor/AdminLTE/dist/img/avatar.png')) . '" alt="" style="width: 100%; height: 100%; object-fit: cover; object-position: center center;" />
        </div>
      ';
      $row[] = $series->content_type;
      $row[] = $series->video_embed;
      $row[] = '
        <a id="' . $this->_context . '-edit-' . $series->content_id . '" type="button" class="btn btn-success btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-edit`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `edit`, ' . $series->content_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-edit"><i class="fa fa-edit"></i> Edit</a>
        <a id="' . $this->_context . '-delete-' . $series->content_id . '" type="button" class="btn btn-danger btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-delete`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `delete`, ' . $series->content_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-delete"><i class="fa fa-trash"></i> Delete</a>
      ';
    ...
    $output = array(
      ...
      "recordsTotal" => $this->series_model->count_all(),
  ...
  public function ajax_get_by_id($id)
  {
    ...
    $row = $this->series_model->getById($id);
  ...
  private function page_meta_data()
  {
    ...
    $data["content"]["table"][$this->_context]["columns"] = $this->series_model->columns();

    $data["content"]["series"]["data"] = $this->series_model->getAll();
                    ...
                    <div class="card-body">
                      ...
                      <div class="form-group">
                        ...
                        <select class="form-control" id="add_content_type" name="content_type">
                          ...
                          <option value="series" ' . set_select("content_type", "series", TRUE) . '>Series</option>
