...
class Tv_show extends CI_Controller
{
  public $security;
  public $db;
  public $session;
  ...
  public $auth_model;
  public $actor_model;
  public $tv_show_model;

  private $_table = 'content';
  private $_context = 'tv_show';
  ...
  public function add()
  {
    ...
    $tv_show = $this->tv_show_model;
    $validation = $this->form_validation;
    $validation->set_rules($tv_show->rules('add'));
    ...
    if (!empty($_FILES[$this->_file[0]]['name'])) :
      ...
      if ($uploadFile['status']) {
        ...
        $tv_show->cover_image = $uploadFile['data']['relative_path'];
      } else {
  ...
  public function edit($id = null)
  {
    ...
    if (!$this->tv_show_model->getById($id)) return show_404();
    ...
    $tv_show = $this->tv_show_model;
    $validation = $this->form_validation;
    $validation->set_rules($tv_show->rules('edit'));
    ...
    if (!empty($_FILES[$this->_file[0]]['name'])) :
      ...
      if ($uploadFile['status']) {
        $old_path = $tv_show->getById($id)->cover_image;
  ...
  public function delete($id)
  {
    ...
    $tv_show_data = $this->tv_show_model->getById($id);

    if (!$tv_show_data) return show_404();

    if ($this->input->post('name_confirm_delete') == $tv_show_data->name) :

      if ($this->tv_show_model->delete($id)) :
  ...
  public function ajax_list()
  {
    ...
    $list = $this->tv_show_model->get_datatables();
    $result = array();
    $row_start = $this->input->post('start');


    //looping data mahasiswa
    foreach ($list as $tv_show) {
      $row_start++;

      $row = array();

      // path to set ajax hit
      $endpoint = $this->_path_prefix . $this->_context . '/ajax_get_by_id/' . $tv_show->content_id;

      $row[] = $tv_show->content_id;
      $row[] = $tv_show->title;
      $row[] = $tv_show->release_year;
      $row[] = $tv_show->duration;
      $row[] = $tv_show->description;
      $row[] = $tv_show->cast;
      $row[] = '
        <div style="width: 7rem; height: 7rem; border-radius: 10px; overflow: hidden;">
          <img src="' . ($tv_show->cover_image ?? base_url('assets/vendor/AdminLTE/dist/img/avatar.png')) . '" alt="" style="width: 100%; height: 100%; object-fit: cover; object-position: center center;" />
        </div>
      ';
      $row[] = $tv_show->content_type;
      $row[] = $tv_show->video_embed;
      $row[] = '
        <a id="' . $this->_context . '-edit-' . $tv_show->content_id . '" type="button" class="btn btn-success btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-edit`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `edit`, ' . $tv_show->content_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-edit"><i class="fa fa-edit"></i> Edit</a>
        <a id="' . $this->_context . '-delete-' . $tv_show->content_id . '" type="button" class="btn btn-danger btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-delete`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `delete`, ' . $tv_show->content_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-delete"><i class="fa fa-trash"></i> Delete</a>
      ';
    ...
    $output = array(
      ...
      "recordsTotal" => $this->tv_show_model->count_all(),
      "recordsFiltered" => $this->tv_show_model->count_filtered(),
      "data" => $result,
    );
  ...
  public function ajax_get_by_id($id)
  {
    ...
    $row = $this->tv_show_model->getById($id);
  ...
  private function page_meta_data()
  {
    ...
    $data["content"]["table"][$this->_context]["columns"] = $this->tv_show_model->columns();

    $data["content"]["tv_show"]["data"] = $this->tv_show_model->getAll();
                    ...
                    <div class="card-body">
                      ...
                      <div class="form-group">
                        ...
                        <select class="form-control" id="add_content_type" name="content_type">
                          <option value="movie" ' . set_select("content_type", "movie") . '>Movie</option>
                          <option value="series" ' . set_select("content_type", "series") . '>Series</option>
                          <option value="tv_show" ' . set_select("content_type", "tv_show", TRUE) . '>Tv Show</option>
                        </select>
