...
class Studio extends CI_Controller
{
  ...
  private $_context = 'studio';
  private $_path_prefix = 'admin/content/';
  private $_file = [];
  ...
  public function add()
  {
    ...
    $validation->set_rules($studio->rules('add'));
  ...
  public function edit($id = null)
  {
    ...
    if (!$this->studio_model->getById($id)) return show_404();
    ...
    $studio = $this->studio_model;
    $validation = $this->form_validation;
    $validation->set_rules($studio->rules('edit'));
  ...
  public function delete($id)
  {
    ...
    $studio_data = $this->studio_model->getById($id);

    if (!$studio_data) return show_404();

    if ($this->input->post('name_confirm_delete') == $studio_data->name) :
  ...
  public function ajax_list()
  {
    ...
    $list = $this->studio_model->get_datatables();
    $result = array();
    ...
    foreach ($list as $studio) {
      ...
      $endpoint = $this->_path_prefix . $this->_context . '/ajax_get_by_id/' . $studio->studio_id;

      $row[] = $studio->studio_id;
      $row[] = $studio->name;
      $row[] = '
        <a id="' . $this->_context . '-edit-' . $studio->studio_id . '" type="button" class="btn btn-success btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-edit`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `edit`, ' . $studio->studio_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-edit"><i class="fa fa-edit"></i> Edit</a>
        <a id="' . $this->_context . '-delete-' . $studio->studio_id . '" type="button" class="btn btn-danger btn-sm text-white m-1" onclick="openModal(`#modal-' . $this->_context . '-delete`, [`' . $this->_path_prefix . '`, `' . $this->_context . '`, `delete`, ' . $studio->studio_id . '], `' . $endpoint . '`);" data-toggle="modal" data-target="#modal-' . $this->_context . '-delete"><i class="fa fa-trash"></i> Delete</a>
      ';
    ...
    $output = array(
      ...
      "recordsTotal" => $this->studio_model->count_all(),
      "recordsFiltered" => $this->studio_model->count_filtered(),
      "data" => $result,
    );
  ...
  public function ajax_get_by_id($id)
  {
    ...
    $row = $this->studio_model->getById($id);
  ...
  public function unique_name_ignore($name, $excludeId)
  {
    ...
    $this->db->where('studio_id !=', $excludeId);
  ...
  private function page_meta_data()
  {
    ...
    $data["content"]["table"][$this->_context]["columns"] = $this->studio_model->columns();

    $data["content"]["studio"]["data"] = $this->studio_model->getAll();
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form id="modal-' . $this->_context . '-add-form" action="' . base_url($this->_path_prefix . $this->_context . "/add") . '" method="post" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="card-body">
                      <input type="hidden" class="form-control" id="add_studio_id" name="studio_id">
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form id="modal-' . $this->_context . '-edit-form" action="' . base_url($this->_path_prefix . $this->_context . "/edit") . '" method="post" enctype="multipart/form-data" autocomplete="off">
                    <input type="hidden" name="' . $data["content"]["csrf"]["name"] . '" value="' . $data["content"]["csrf"]["hash"] . '">
                    <div class="card-body">
                      <input type="hidden" class="form-control" id="edit_studio_id" name="studio_id">
                      <div class="form-group">
                        <label for="name">Name *</label>
