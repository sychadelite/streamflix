      ...
      <div class="col-12">
        ...
        <div class="card">
          <div class="card-header d-flex flex-wrap justify-content-between align-items-center" style="gap: 8px;">
            <h3 class="card-title">Manage <?= $context ?> with data table</h3>
  ...
  $(document).ready(function() {
    ...
    $(function() {
      var dt = $("#<?= $context ?>_data_table").DataTable({
        ...
        "serverSide": true,
        "ajax": {
          "url": '<?= base_url("admin/" . $context . "/ajax_list"); ?>',
          "type": "POST",
          "data": function(d) {
    ...
    $(function() {
      ...
      <?php endif; ?>
      <?php if ($this->session->flashdata("message_upload_file_error")) : ?>
        toastr.error('<?= $this->session->flashdata("message_upload_file_error") ?>');
      <?php endif; ?>
      <?php if ($this->session->flashdata("message_remove_file_error")) : ?>
        toastr.error('<?= $this->session->flashdata("message_remove_file_error") ?>');
      <?php endif; ?>
    })
  });
</script>